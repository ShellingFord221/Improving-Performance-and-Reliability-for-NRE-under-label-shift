...
class ModelWithTemperature(nn.Module):
    ...
    def __init__(self, model, label_num, device):
        ...
        self.temperature = nn.Parameter(torch.ones(label_num))
    ...
    # 用于记下最佳参数
    def set_best_parameter(self, logits, temp):
        self.optimal_temp = temp.cuda()
        # self.optimal_bias = bias.cuda()
        return logits * self.optimal_temp
    ...
    def set_temperature(self, dset):
        ...
        new_logits = self.set_best_parameter(logits, optimal_temp)
        ...
        print('Optimal temperature: ', self.optimal_temp.detach().cpu().numpy())
        ...
        # ece_calibrated = self.make_model_diagrams(new_logits, labels, fig_name + '_calibrated')
